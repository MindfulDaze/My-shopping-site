...
function displayItems(filter_id)
{
	...
	fetch("myproducts.json").then((response)=>
		response.json()
		).then((myObject)=>
		{
			...
			if(myObject[k].filter_id===filter_id)
				{
					...
					document.querySelectorAll(".myRow")[0].style.width="1000px"
					...
					document.querySelectorAll(".myRow")[0].style.padding="3px"
					document.querySelectorAll(".myRow")[0].style.boxsizing="border-box"
					document.querySelectorAll(".myRow")[0].style.margin="auto"
					document.querySelectorAll(".myRow")[0].style.color="#ADFDA2"
					...
					for(let i in myBlocks)
					{
						...
						myBlocks[i].style.boxsizing="border-box"	
						myBlocks[i].style.margin="auto"	
						myBlocks[i].style.padding="auto"
						myBlocks[i].style.margin="5px"	
					...
					if(!(String(myCartButton[j].innerHTML)==="undefined")&&(typeof(myCartButton[j])==="object"))
						...
						myCartButton[j].style.borderRadius="7px"
						myCartButton[j].style.backgroundcolor="grey"
...
function displayProduct(product)
{
	...
	<br>
	<HR>
	<span>₹${product.price}</span><button onclick="addToCart(${product.product_id},'${product.product_name}','${product.imgSrc}',${product.price})">AddCart</button></div>`
...
function addToCart(product_id,product_name,imgSrc,price)
{
	...
	let inputHTML=`<input style="margin-left:10px;text-align:center;" type="number"  id="i${product_id}" value="1" onchange="populateQuantity(${false})">`
	...
	returnDiv=`<div id="flexContainer">
			   ...
			   <hr><button class="badge badge-pill badge-danger" style="padding:10px; background-color:skyblue;" onclick="removeItem(${product_id})"><i class="bi bi-trash3-fill"></i></button></div><div id="buttons" style="background-color:none;background-image:none;border:none;"><button class="btn btn-danger" onclick="clearCart()" style="float:left;margin:5px auto auto 5px;">Clear Cart</button>
			   <button class="btn btn-success" style="float:right;margin:5px 5px auto auto;" onclick="checkOut()" >Check-Out</button></div></div>`
...
function  checkOut()
	{
		...
		grandTotal+=tot
		myTable+=`<tr><td class="table-product-content"><img src="${myImageArr[i]}" style="width:140px;height:100px;object-fit:cover;border:1px solid white;"></td><td class="table-product-items">${qtyArr[i]}</td><td class="table-product-items">₹${priceArr[i]}</td><td class="table-product-total">₹${tot}</td></tr>`
	...
	{
	let cardDiv=`<img src="image/america.jpg"><img src="image/master.jpg"><img src="image/visa.jpg"><img src="image/gold1.jpg">`
	cardDiv+=`<H4 style="margin:10px auto auto auto">Amount Payable:₹${total}</H4>`
	
	cardDiv+=`<div style="width:510px;margin:auto;"><div style="float:left;margin:5px auto auto auto;"><label>Card Number:</label><input type="text" style="width:50px;text-align:center;" maxlength="4">-<input type="text" style="width:50px;text-align:center;" maxlength="4">-<input type="text" style="width:50px;text-align:center;" maxlength="4">-<input type="text" style="width:50px;text-align:center;" maxlength="4"></div>`
	cardDiv+=`<div style="margin:10px 10px auto auto; margin-right:100px;float:right;">CVV/CVV2:<input type="password" style="appearance:none;padding-left:15px;margin-right:70px;width:70px;height:30px;background-image:url('image/lock.jpg');background-size:22px auto;background-position:1px 5px;background-repeat:no-repeat;" maxlength="3"></div></div>`
	...
	cardDiv+=`<button class="btn btn-success" onclick="paymentSuccessfull()" style="float:right;margin-right:15px;margin-top:3px;">Confirm payment</button>`
...
function removeItem(product_id)
{
	...
	priceArr=localStorage.getItem("price").split(",")
	...
	priceArr.splice(removePosition,1)
		...
		localStorage.setItem("price",priceArr)
